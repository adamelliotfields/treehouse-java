package io.github.adamelliotfields.jobs.service;
...
import io.github.adamelliotfields.jobs.model.ResultsPage;
...
public class JobService {
  ...
  public List<HttpResponse> search() throws IOException {
    ...
    ResultsPage estimate = requestFactory.buildGetRequest(estimateUrl)
                                         .execute()
                                         .parseAs(ResultsPage.class);

    System.out.println("Estimated results: " + estimate.totalResults);
    return IntStream.range(0, Math.min(estimate.totalResults, 1000))
                    .parallel()
